...
function prfx_field_meta_service_callback( $post ) {
    ...
    <p>
        ...
        <input type="text" name="opt-service-link" id="link" value="<?php if ( isset ( $prfx_stored_meta['opt-service-link'] ) ) echo $prfx_stored_meta['opt-service-link'][0]; ?>" />
    </p>
    <?php
}
...
function prfx_field_service_save( $post_id ) {
    ...
    if( isset( $_POST['opt-service-link'] ) ) {

        update_post_meta( $post_id, 'opt-service-link', $_POST[ 'opt-service-link' ] );    
