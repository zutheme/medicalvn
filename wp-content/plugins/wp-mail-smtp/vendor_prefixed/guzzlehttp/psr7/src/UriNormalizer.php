...
final class UriNormalizer
{
    ...
    public static function normalize(\WPMailSMTP\Vendor\Psr\Http\Message\UriInterface $uri, $flags = self::PRESERVING_NORMALIZATIONS)
    {
        ...
        if ($flags & self::REMOVE_DEFAULT_PORT && $uri->getPort() !== null && \WPMailSMTP\Vendor\GuzzleHttp\Psr7\Uri::isDefaultPort($uri)) {
            $uri = $uri->withPort(null);
        }
        if ($flags & self::REMOVE_DOT_SEGMENTS && !\WPMailSMTP\Vendor\GuzzleHttp\Psr7\Uri::isRelativePathReference($uri)) {
            $uri = $uri->withPath(\WPMailSMTP\Vendor\GuzzleHttp\Psr7\UriResolver::removeDotSegments($uri->getPath()));
        }
    ...
    private static function capitalizePercentEncoding(\WPMailSMTP\Vendor\Psr\Http\Message\UriInterface $uri)
    {
        $regex = '/(?:%[A-Fa-f0-9]{2})++/';
    ...
    private static function decodeUnreservedCharacters(\WPMailSMTP\Vendor\Psr\Http\Message\UriInterface $uri)
    {
