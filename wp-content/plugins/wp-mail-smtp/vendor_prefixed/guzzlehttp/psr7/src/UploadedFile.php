...
class UploadedFile implements \WPMailSMTP\Vendor\Psr\Http\Message\UploadedFileInterface
{
    /**
     * @var int[]
     */
    ...
    private function setStreamOrFile($streamOrFile)
    {
        if (\is_string($streamOrFile)) {
            ...
            $this->stream = new \WPMailSMTP\Vendor\GuzzleHttp\Psr7\Stream($streamOrFile);
        } elseif ($streamOrFile instanceof \WPMailSMTP\Vendor\Psr\Http\Message\StreamInterface) {
    ...
    private function setError($error)
    {
        ...
        if (\false === \in_array($error, \WPMailSMTP\Vendor\GuzzleHttp\Psr7\UploadedFile::$errors)) {
    ...
    {
        $this->validateActive();
        if ($this->stream instanceof \WPMailSMTP\Vendor\Psr\Http\Message\StreamInterface) {
            return $this->stream;
        }
        return new \WPMailSMTP\Vendor\GuzzleHttp\Psr7\LazyOpenStream($this->file, 'r+');
    }
    /**
     * {@inheritdoc}
     *
