 ...
 */
namespace WPMailSMTP\Vendor\phpseclib\Crypt;
...
class RSA
{
    ...
    function __construct()
    {
        ...
        $this->one = new \WPMailSMTP\Vendor\phpseclib\Math\BigInteger(1);
    ...
    {
        if (!\defined('WPMailSMTP\\Vendor\\CRYPT_RSA_EXPONENT')) {
        ...
        if (!\defined('WPMailSMTP\\Vendor\\CRYPT_RSA_SMALLEST_PRIME')) {
            \define('WPMailSMTP\\Vendor\\CRYPT_RSA_SMALLEST_PRIME', 4096);
        }
        ...
        static $e;
        if (!isset($e)) {
            $e = new \WPMailSMTP\Vendor\phpseclib\Math\BigInteger(CRYPT_RSA_EXPONENT);
        }
        ...
        $generator = new \WPMailSMTP\Vendor\phpseclib\Math\BigInteger();
        $n = $this->one->copy();
        if (!empty($partial)) {
    ...
    function _stop_element_handler($parser, $name)
    {
        if (isset($this->current)) {
            $this->current = new \WPMailSMTP\Vendor\phpseclib\Math\BigInteger(\base64_decode($this->current), 256);
            unset($this->current);
        }
    }
    /**
    ...
    {
        if ($key instanceof \WPMailSMTP\Vendor\phpseclib\Crypt\RSA) {
            ...
            if (\is_object($key->hash)) {
                $this->hash = new \WPMailSMTP\Vendor\phpseclib\Crypt\Hash($key->hash->getHash());
            }
            if (\is_object($key->mgfHash)) {
                $this->mgfHash = new \WPMailSMTP\Vendor\phpseclib\Crypt\Hash($key->mgfHash->getHash());
            }
    ...
    function _generateMinMax($bits)
    {
        ...
        return array('min' => new \WPMailSMTP\Vendor\phpseclib\Math\BigInteger($min, 256), 'max' => new \WPMailSMTP\Vendor\phpseclib\Math\BigInteger($max, 256));
    }
    /**
    ...
    function _os2ip($x)
    {
        return new \WPMailSMTP\Vendor\phpseclib\Math\BigInteger($x, 256);
    }
    /**
    ...
    function _exponentiate($x)
    {
        ...
        if (\defined('WPMailSMTP\\Vendor\\CRYPT_RSA_DISABLE_BLINDING')) {
            ...
            $one = new \WPMailSMTP\Vendor\phpseclib\Math\BigInteger(1);
    ...
    function _rsaes_oaep_encrypt($m, $l = '')
    {
        ...
        $db = $lHash . $ps . \chr(1) . $m;
        $seed = \WPMailSMTP\Vendor\phpseclib\Crypt\Random::string($this->hLen);
    ...
    function _rsaes_pkcs1_v1_5_encrypt($m)
    {
        ...
        while (\strlen($ps) != $psLen) {
            $temp = \WPMailSMTP\Vendor\phpseclib\Crypt\Random::string($psLen - \strlen($ps));
            $temp = \str_replace("\0", '', $temp);
            $ps .= $temp;
        }
        $type = 2;
        // see the comments of _rsaes_pkcs1_v1_5_decrypt() to understand why this is being done
        if (\defined('WPMailSMTP\\Vendor\\CRYPT_RSA_PKCS15_COMPAT') && (!isset($this->publicExponent) || $this->exponent !== $this->publicExponent)) {
            $type = 1;
    ...
    function _emsa_pss_encode($m, $emBits)
    {
        ...
        $salt = \WPMailSMTP\Vendor\phpseclib\Crypt\Random::string($sLen);
