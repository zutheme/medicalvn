...
class Google_Service_Apigee_Resource_OrganizationsDevelopersApps extends Google_Service_Resource
{
   ...
   * to use in calls to API proxies inside that API product.
   *
   * The `name` is the unique ID of the app that you can use in API calls. The
   * `DisplayName` (set as an attribute) appears in the UI. If you don't set the
   * `DisplayName` attribute, the `name` appears in the UI. (apps.create)
   *
   ...
   *
   * **Note**: The delete operation is asynchronous. The developer app is deleted
   * immediately, but its associated resources, such as app keys or access tokens,
   ...
   *
   * * Approve or revoke a developer app * Generate a new consumer key and secret
   * for a developer app
   *
   ...
   * Content`
   *
   * To generate a new consumer key and secret for a developer app, pass the new
   * key/secret details. Rather than replace an existing key, this API generates a
   * new key. In this case, multiple key pairs may be associated with a single
   * developer app. Each key pair has an independent status (`approved` or
   * `revoked`) and expiration time. Any approved, non-expired key can be used in
   * an API call.
   *
   ...
   * is compromised.
   *
   * The `keyExpiresIn` property defines the expiration time for the API key in
   * milliseconds. If you don't set this property or set it to `-1`, the API key
   * never expires.
   *
   * **Notes**:
   *
   * * When generating a new key/secret, this API replaces the existing
   * attributes, notes, and callback URLs with those specified in the request.
   * Include or exclude any existing information that you want to retain or
   * delete, respectively. * To migrate existing consumer keys and secrets to
   ...
   * you can request an expanded view of the developer apps.
   *
   ...
   * parameters. (apps.listOrganizationsDevelopersApps)
   *
   * @param string $parent Required. Name of the developer. Use the following
   * structure in your request:
   * `organizations/{org}/developers/{developer_email}`
   ...
   * developer apps. For example, if you're returning 50 developer apps at a time
   * (using the `count` query parameter), you can view developer apps 50-99 by
   * entering the name of the 50th developer app. The developer app name is case
   * sensitive.
   ...
   * to use when calling APIs in the API product.
   *
   * If you want to use an existing API key for the API product, add the API
   * product to the API key using the UpdateDeveloperAppKey API.
   *
   ...
   * request. Include or exclude any existing attributes that you want to retain
   * or delete, respectively. (apps.update)
   *
   * @param string $name Required. Name of the developer app. Use the following
   * structure in your request:
   * `organizations/{org}/developers/{developer_email}/apps/{app}`
